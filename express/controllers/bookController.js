...
exports.book_delete_post = function(req, res) {
    async.parallel({
        ...
        book_books: function(callback) {
          author.find({ 'book': req.body.authorid }).exec(callback)
        },
    }, function(err, results) {
        if (err) { return next(err); }
        // Success
        if (results.book_books.length > 0) {
            ...
            res.render('book_delete', { title: 'Delete book', book: results.book, book_books: results.book_books } );
            return;
        }
        else {
            // Books has no books. Delete object and redirect to the list of authors.
